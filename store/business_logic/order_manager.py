...
def addProductToCartStatus(user_id, id, json):
    ...
    if  OrderProduct.get_order_product(order.id, id,1):
        OrderProduct.updateSumQuantity(order.id, id,1, json['value'])
    else:
        OrderProduct.add_order_product(order.id, id,1, json['value'])